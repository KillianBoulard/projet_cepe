...
```{r setup}
knitr::opts_knit$set(root.dir = "c:/Users/User/Desktop/Cours et documents/formations/formation R ensae/DATA")
knitr::opts_knit$set(root.dir = "C:/Users/VOYK743/Desktop/Fichiers Perso/Formation ENSAI/Datasets")
knitr::opts_knit$set(root.dir = "C:/Users/killi/Documents/dataset")
```
...
inc_dataset = incendies %>% 
 mutate(code_insee = if_else(nchar(code_insee) == 4,paste0("0",code_insee),code_insee)) %>%
  ...
  mutate(
    date_alerte = as.Date(date_alerte, format = "%d/%m/%Y"),#%Y-%m-%d
...
synt_gen_comm_dataset = base_dataset %>% 
...
arrange(annee,mois) %>%
  mutate (
       moy_surface_parcourue = lag(round(cummean(surface_parcourue),3)),
        ...
        moy_surface_agricole        = lag(round(cummean(surface_agricole),3)),
        ...
        moy_orig_alerte_ind   = lag(cummean(orig_alerte_ind)),
...
synt_gen_comm_dataset = synt_gen_comm_dataset %>% 
  group_by(annee,mois,code_insee) %>% 
filter(row_number()==1)

synt_gen_comm_dataset <- synt_gen_comm_dataset %>% replace(is.na(.), 0)
...
METEO_QUANTI <- meteo %>%  
  ...
  arrange(annee_mesure,mois_mesure,heure_mesure)
...
#Pour les données météo générales, on remplace par la dernière valeur connue
base_dataset_full = base_dataset_full %>% 
  group_by(id_station) %>%
  fill(c(ends_with("23_g"), ends_with("14_g")), .direction = "up") %>% 
  ungroup()
...
index1 = as.numeric(match("pression_niveau_mer_14_g", names(base_dataset_full)))
index2 = as.numeric(match("precipitation_12dh_23_g", names(base_dataset_full)))

indexv1 = as.numeric(match("pression_niveau_mer_14_m_1", names(base_dataset_full)))
indexv2 = as.numeric(match("precipitation_12dh_23_m_1", names(base_dataset_full)))

indexv3 = as.numeric(match("pression_niveau_mer_14_m_12", names(base_dataset_full)))
indexv4 = as.numeric(match("precipitation_12dh_23_m_12", names(base_dataset_full)))

indexv5 = as.numeric(match("pression_niveau_mer_14_m", names(base_dataset_full)))
indexv6 = as.numeric(match("precipitation_12dh_23_m", names(base_dataset_full)))

ii = index1
var = indexv1 
  
#Pour les données météo manquantes (m1 et m12) : application de la donnée générale moyenne sur la station
 for (i in 1:84){
   varname = colnames(base_dataset_full)[var]
    #print(paste0(varname," // ",colnames(base_dataset_full)[ii]))
    base_dataset_full = base_dataset_full %>%
...
dataset = base_dataset_full %>% 
  ...
  inner_join(inter, by="code_insee")
...
#write.csv(dataset, row.names=T)
#write.csv2(dataset, row.names=T)
