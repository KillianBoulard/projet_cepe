...
synt_gen_comm_dataset <-inc_dataset %>%
  ...
  group_by(code_insee) %>%
  mutate ( 
        moy_surface_foret_g  = round(mean(surface_foret),3),
        moy_surface_maquis_g = round(mean(surface_maquis),3),
        moy_surface_nat_autre_foret_g = round(mean(surface_nat_autre_foret),3),
        moy_surface_agricole_g        = round(mean(surface_agricole),3),
        moy_surface_autre_terre_boisee_g  = round(mean(surface_autre_terre_boisee),3),
        moy_surface_non_boisee_nat_g      = round(mean(surface_non_boisee_nat),3),
        moy_surface_non_boisee_art_g      = round(mean(surface_non_boisee_art),3),
        moy_suface_non_boisee_g           = round(mean(suface_non_boisee),3),
    
        moy_nature_ind_g      = 	mean(nature_ind),
        moy_nature_acc_g      = 	mean(nature_acc),
        moy_nature_inv_part_g = 	mean(nature_inv_part),
        moy_nature_inv_trav_g = 	mean(nature_inv_trav),
        moy_nature_nat_g      = 	mean(nature_nat),
        moy_nature_malv_g     = 	mean(nature_malv), 
    
        moy_orig_alerte_autr_g  = mean(orig_alerte_autr ),
        moy_orig_alerte_ind_g   = mean(orig_alerte_ind  ),
        moy_orig_alerte_mae_g   = mean(orig_alerte_mae  ),
        moy_orig_alerte_pat_g   = mean(orig_alerte_pat  ),
        moy_orig_alerte_ppr_g   = mean(orig_alerte_ppr  ),
        moy_orig_alerte_pop_g   = mean(orig_alerte_pop  ),
        moy_orig_alerte_vcam_g  = mean(orig_alerte_vcam )
  )  %>%

  distinct(code_insee,
           ###info surface par commune
           moy_surface_foret_g,
           moy_surface_maquis_g,
           moy_surface_nat_autre_foret_g,                               
           moy_surface_agricole_g,
           moy_surface_autre_terre_boisee_g,
           moy_surface_non_boisee_nat_g,
           moy_surface_non_boisee_art_g,
           moy_suface_non_boisee_g,
           ###info nature incendie
           moy_nature_ind_g,
           moy_nature_acc_g,
           moy_nature_inv_part_g,
           moy_nature_inv_trav_g,
           moy_nature_nat_g,
           moy_nature_malv_g,
           ###info origine incendie
           moy_orig_alerte_autr_g, moy_orig_alerte_ind_g  ,moy_orig_alerte_mae_g  ,
           moy_orig_alerte_pat_g,  moy_orig_alerte_ppr_g , moy_orig_alerte_pop_g , moy_orig_alerte_vcam_g ) %>% 
           select(-moy_surface_agricole_g) %>%
           ungroup()
```

### Etape 5.2 : Synthèse du fichier incendie par code insee / année / mois

```{r}
synt_mois_comm_dataset <-inc_dataset %>%
  select(annee,code_insee,origine_alerte,surface_parcourue,surface_foret,surface_maquis,
         surface_nat_autre_foret,surface_agricole,surface_autre_terre_boisee,surface_non_boisee_nat,
         surface_non_boisee_art,suface_non_boisee,nature, mois) %>% 
  select(- c(surface_parcourue))                   %>%
  mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .)))     %>%
  mutate(
          nature_ind = ifelse(nature=="",1,0) ,
          nature_acc = ifelse(nature=="Accidentelle",1,0),
...
### Etape 5.3 : synthèse générale du fichier météo par station

```{r}
prop_na <-meteo %>% summarize_all(funs(sum(is.na(.)) / length(.)))
prop_na <-transpose %>% summarize_all(funs(sum(is.na(.)) / length(.)))
...
test = METEO_QUANTI %>% 
  distinct(id_station, mois_mesure, annee_mesure) %>% 
  summarise(n = n())


test = METEO_QUANTI %>% 
  group_by(heure_mesure) %>% 
  summarise(nb = n()) %>% 
  select(heure_mesure, nb) %>% 
  ungroup()
