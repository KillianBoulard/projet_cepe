...
```{r}
communes <- read.csv(file="laposte_hexasmal.csv", header = T, sep=";",  encoding="UTF-8")
com_dataset = communes %>% 
                    select(code_insee = Code_commune_INSEE,
                           nom_commune = Nom_commune ,
                           gps = coordonnees_gps)
rm(communes)
```

## Création d'une matrice janvier 2011 à décembre 2019

```{r}
#Année
annee = as.data.frame(matrix(2011:2019, nrow = 9, ncol = 1))
annee=annee %>% 
  rename("annee" = "V1") %>% 
