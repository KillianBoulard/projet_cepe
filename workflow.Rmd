...
### Etape 5.2 : traitement du fichier incendie
...
transpose_by_station_annee_mois = METEO_QUANTI %>%
  ...
  pivot_wider(
    ...
    values_fn = mean) %>% 
  ungroup()  %>% 
  mutate(id_station = as.integer(id_station))
...
x<- sample(1:nrow(base_dataset), 100000)
base_dataset_test = base_dataset[x, ]

test = base_dataset_test %>% 
    left_join(synt_gen_comm_dataset, keep= T,suffix = c('_1', '_2'), 
          by=c("code_insee")) %>% 
      left_join(transpose_by_station, keep= F, 
          by=c("id_station"))
