...
x <- sample(1:nrow(training_set), 100)
training_set_echant = training_set[x, ]


preProcess <- c("center","scale")
trControl <- trainControl(method = "repeatedcv",number = 2,repeats = 2)

model <- train(Y ~ ., method='knn', data = training_set_echant, metric='Accuracy',preProcess = preProcess, trControl=trControl)
